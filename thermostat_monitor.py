...
# TODO
# refill deque from saved info on disk to avoid outliers in initial readings
# Improve retrying code to eventually give up and continue logging to disk, trying again later.
# add support for using the CT/INT pins of the temperature sensor

# ADT7410 tick constants
...
def get_config(config_file):
	...
	config_dict['window'] = config.getint('general', 'window')
	config_dict['threshold'] = config.getfloat('general', 'threshold')
	config_dict['reset'] = config.getint('general', 'reset')
	return config_dict
...
def normal_temp_read(i2c_addr, bus=1):
	# Configure device in one sample per second mode if not already in it
	bus = config_dict['i2c_bus']
	if get_dev_conf(i2c_addr) != ADT7410_CONF_SPS >> 5:
		try:
			...
			# Wait for double the datasheet spec time
			time.sleep(0.120)
		except subprocess.CalledProcessError as e:
			print "hipi-i2c subprocess error code: ", e.returncode
			print "hipi-i2c subprocess output: ", e.output
...
def one_shot_temp_read(i2c_addr):
	# Note that the config register needs to be set to OSM every time we do a OSM reading
	bus = config_dict['i2c_bus']
	try:
		subprocess.check_output(["/usr/local/bin/hipi-i2c", "w", "{}".format(bus), "0x{:02X}".format(i2c_addr), "0x{:02X}".format(ADT7410_CONF), "0x{:02X}".format(ADT7410_CONF_SPS)])
		#print "DEBUG current device configuration: {:#b}".format(get_dev_conf(i2c_addr))	
		# ADT7410 datasheet says to wait at least 240 ms after setting OSM before reading; here we wait double that
		time.sleep(0.480)
	except subprocess.CalledProcessError as e:
		print "hipi-i2c subprocess error code: ", e.returncode
		print "hipi-i2c subprocess output: ", e.output
...
def is_outlier(temp):
	...
	if len(is_outlier.mad) == is_outlier.window:
		...
		z = abs_dev / mad
		print z
		# We should multiply the threshold by the number of misses to account for larger changes after >1 miss
		if z >= (is_outlier.threshold * (is_outlier.misses + 1)):
			is_outlier.misses += 1
...
def cb_therm_status(channel, prev_therm_status = None, interrupt=True):
	...
	if (temp is not None) and (is_outlier(temp) == False):
		...
		temp = None # Set outlier temperatures to None so they won't be graphed
	f.flush()
...
def graph_therm(timestamp, therm_status, temp, prev_therm_status = None, interrupt=False):
	...
	# Make the x-axis range the prior day plus 6 hours (should correct time zone)
	x_range = [to_unix_time(timestamp - datetime.timedelta(days=1)  - datetime.timedelta(microseconds=timestamp.microsecond)), to_unix_time(timestamp  + datetime.timedelta(hours=6) - datetime.timedelta(microseconds=timestamp.microsecond))]	
	# If this reading was generated by an interrupt, create a pseudo-reading
	...
	layout = Layout(
					...
					xaxis=XAxis(title='Date and Time', autorange=False, range=x_range)
					)
...
def main():
	try:
		...
		is_outlier.window = config_dict['window'] # set the window for median calculations
		is_outlier.threshold = config_dict['threshold'] # set the threshold multiplier for detecting outliers
		is_outlier.misses = 0
		is_outlier.reset = config_dict['reset'] # threshold for consecutive misses before resetting deques
